...
if(eris === djs) {
  if(eris)
    throw ReferenceError("Both eris and discord.js are installed, please uninstall one.");
  else
    throw ReferenceError("Neither eris nor discord.js are installed, please install one.");
}else if(eris) {
  funcs.isPerm = permission => {
    if(perms == null)
      perms = Object.keys(require("eris").Constants.Permissions);
  ...
  funcs.fetchMember = function(message, id) {
    const member = message.channel.guild.get(id);
  ...
  funcs.isPerm = permission => {
    if(perms == null)
      perms = Object.keys(require("discord.js").Permissions.FLAGS);
