...
export default function(registry) {
  ...
  GroupReader.read = function(input, command, _, argument) {
    const value = input.toLowerCase();
    const match = registry.groups.find(group => group.name === input);
    ...
    return ReaderUtil.handleMatches(
      command,
      registry.groups.filter(group => group.name.toLowerCase().includes(value)),
