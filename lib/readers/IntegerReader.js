 ...
 */
const IntegerReader = new TypeReader({type: "integer"});

IntegerReader.default = true;
IntegerReader.read = function(input, command, message, argument) {
  const value = Number(input);

  if(!Number.isInteger(value))
    return readerUtil.handleFailure(command, message, argument, `You've provided an invalid ${argument.name}.`);
  else if(value > Number.MAX_SAFE_INTEGER)
    return readerUtil.handleFailure(command, message, argument, "That number is too large.");
  else if(value < Number.MIN_SAFE_INTEGER)
    return readerUtil.handleFailure(command, message, argument, "That number is too small.");
