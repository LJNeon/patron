...
class Registry {
  constructor(options) {
    ...
    this.defaultReaders = options.defaultReaders ?? true;
  ...
  static setLibrary(library) {
    if(library === "eris" || library === "discordjs")
      Registry.library = require(`../utils/${library}Handler.js`);
    else
      throw TypeError("The library must be either 'eris' or 'discordjs'.");
  }

  static getLibrary() {
    if(Registry.library == null)
      throw ReferenceError("Patron requires a library to be set in order to function.");
  ...
  registerPrefixes(prefixes, guildId = "global") {
    ...
    const results = existing?.concat(prefixes) ?? prefixes;
  ...
  registerArgumentPreconditions(conditions) {
    ...
    if(!Array.isArray(conditions) || conditions.some(c => !(c instanceof ArgumentPrecondition)))
      throw TypeError("Registry#registerArgumentPreconditions() only accepts an array of ArgumentPreconditions.");
  ...
  unregisterTypeReaders(types) {
    ...
    for(let i = 0; i < types.length; i++) {
      ...
      if(!this.typeReaders.delete(type))
