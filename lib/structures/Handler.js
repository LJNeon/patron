...
class Handler {
  ...
  parseCommand(message) {
    let prefix = this.prefixes.get("global").find(p => message.content.startsWith(p));

    if(prefix == null && message.channel.guild != null)
      prefix = this.prefixes.get(message.channel.guild.id).find(p => message.content.startsWith(p));
