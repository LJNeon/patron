...
class Handler {
  ...
  parsePrefix(message) {
    ...
    if(message.channel.guild != null) {
      ...
      if(prefixes != null) {
        ...
        if(prefix != null)
          return prefix === "@mention" ? client.user.id.length + 4 : prefix.length;
        else
          return;
      }
    }
  ...
  parseCommand(message) {
    const prefixLength = this.parsePrefix(message);
  ...
  async useCooldown(message, command) {
    let cooldown = await command.useCooldown(message.author.id, message.channel.guild?.id);
    let group;
    ...
    if(!cooldown) {
      ...
      cooldown = await group.useCooldown(message.author.id, message.channel.guild?.id);

      if(!cooldown)
        return;
    }

    cooldown = await (group ?? command).getCooldown(message.author.id, message.channel.guild?.id);

    return CooldownResult.fromFailure(command, cooldown.reset - Date.now(), group);
  }

  async revertCooldown(message, command) {
    await command.revertCooldown(message.author.id, message.channel.guild?.id);

    if(command.group != null)
      await this.registry.getGroup(command.group).revertCooldown(message.author.id, message.channel.guild?.id);
  }
