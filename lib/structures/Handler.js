...
class Handler {
  ...
  async runArgumentPreconditions(message, command, info, parsed, value) {
    for(let i = 0; i < info.preconditions.length; i++) {
      const result = await this.registry.getArgumentPrecondition(info.preconditions[i])
  ...
  async validateArgument(message, command, info, parsed, input) {
    let result = await this.registry.getTypeReader(info.type)
      .read(input, command, message, info, parsed, info.typeOptions);
