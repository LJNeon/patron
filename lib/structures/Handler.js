...
class Handler {
  ...
  async parseArguments(message, command, args) {
    ...
    for(let i = 0; i < command.arguments.length; i++) {
      ...
      if(info.infinite) {
        ...
        const options = [info, command.arguments[i + 1]];

        for(let j = i + 2; j < command.arguments.length; j++) {
          if(command.arguments[j].optional)
            options.push(command.arguments[j]);
          else
            break;
        }

        const size = i + options.length;

        for(let j = i; i < size; i++) {
          const input = options[i].remainder ? args.slice(j).join(this.separator) : args[j];
          const result = await this.validateArgument(message, command, options[i], parsed, input);

          if(result instanceof TypeReaderResult)
            parsed[options[i].key] = this.default(message, options[i].defaultValue);
  ...
  async run(message, prefixLength) {
    const result = this.parseCommand(message, prefixLength);
