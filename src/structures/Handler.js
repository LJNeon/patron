...
class Handler {
	...
	async run(context, prefix) {
		...
		let command = registry.commands.get(commandName);
		
		if (command === undefined) {
			const matches = registry.commands.filterArray((value) => value.aliases.some((v) => v === commandName));
		...
		for (let i = 0; i < command.args.length; i++) {
			...
			const typeReaderResult = await registry.typeReaders.get(command.args[i].type).read(command, context, command.args[i], input);
			
			if (!typeReaderResult.isSuccess) {
				return result;
			}
			
			for (precondition of result.value.preconditions) {
		...
		for (precondition of command.group.preconditions.concat(command.preconditions)) {
